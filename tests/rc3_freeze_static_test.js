...
function main() {
    assertContains('version.php', /\$plugin->release\s*=\s*'(?:1.3.78-rc3|1.3.79|1.3.8[0-3])'/, 'rc3 release marker');
    assertContains('version.php', /\$plugin->maturity\s*=\s*MATURITY_(?:RC|STABLE)/, 'release-candidate maturity');
    assertContains('version.php', /\$plugin->version\s*=\s*202605250(?:7[89]|8[0-3]);/, 'incremented plugin version');
