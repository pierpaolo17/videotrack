...
## Integrity indicators and focus controls (1.6.18)

Activities can optionally record bounded diagnostic signals and enable focus controls, best-effort Picture-in-Picture prevention and random attention pauses. Site administrators define the random interval (default 300–1800 seconds) and choose an accessibility-oriented or strict window-focus policy. A hidden video tab can pause playback; ordinary window blur is recorded after a grace period and pauses only in strict mode. Analytics always shows the integrity section and explains when recording is disabled. Signals contain no webcam, microphone, biometrics, free text or content from other tabs; teacher output is contextual, privacy-thresholded and must not be treated as proof of misconduct. See `docs/en/11_INTEGRITY_AND_FOCUS.md` and `docs/it/11_INTEGRITY_AND_FOCUS.md`.
