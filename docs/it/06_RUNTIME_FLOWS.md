...
Le definizioni delle reazioni sono configurate in un’unica sezione richiudibile **Reazioni**. La funzione parte disabilitata e per abilitarla serve almeno una definizione attiva completa. Il form mostra almeno quattro righe modificabili, mantiene una riga libera dopo le definizioni esistenti e può essere esteso con **Aggiungi reazione** fino al limite di 30. Controlli e cronologia personale compaiono soltanto quando sono presenti sia l’abilitazione sia una definizione. I learner canonici possono attivare le reazioni durante riproduzione o pausa; prima dell’interazione il client esegue il flush del progresso e usa l’estremo salvato restituito dal server. I timestamp di reazioni, note, segnalibri e Forum sono accettati soltanto se coperti da evidenza di visione validata dal server; il permesso di seek avanti non rende mai autorevole una riga `playstart`. Chi può consultare i report riceve un’anteprima visibile ma disabilitata e non può salvare telemetria learner. Note, segnalibri e Forum restano governati in modo indipendente dai rispettivi flag e dalla destinazione. I servizi verificano abilitazione, scope learner, proprietà e lunghezze massime. I record eliminati mantengono una tombstone limitata fino a retention/cancellazione; il testo privato non entra negli aggregati docente.
...
I report per studente includono soltanto utenti con `mod/videotrack:participate` e possono mostrare identità, progresso, reazioni, data/fotografia della presa visione e conteggi diagnostici solo con accesso ai report individuali. Chi dispone del solo accesso aggregato non può filtrare per studente e mantiene il masking minimo configurato. Le dashboard aggregano attività accessibili. La dashboard di corso usa renderer distinti per percentuali testuali e percentuali con barra compatta accessibile. Gli Analytics di istanza usano bin e riepiloghi evento esatti soltanto quando l’accesso individuale copre tutto lo scope selezionato; altrimenti applicano la soglia configurata. I download aggregati CSV/Excel/ODS mantengono lo stesso livello di privacy. Il relativo costruttore delle intestazioni espone entry point distinti per l’export standard e quello con riepiloghi evento, rendendo esplicita la forma selezionata nel chiamante. Gli export CSV individuali richiedono il permesso dedicato di export individuale e conferma esplicita quando contengono dati personali.
