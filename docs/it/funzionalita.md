...
**Versione**: 1.4.143 (build 2026060266)
...
- Documentati nel codice e in `docs/it/ajax-layer.md` i limiti operativi AMD relativi ad AJAX, retry e beacon.
...
La release 1.4.94 non modifica le funzionalità didattiche. Introduce solo un micro-refactor interno del layer AJAX per separare il trasporto core/ajax e il timeout client-side in un modulo AMD dedicato.


## Aggiornamento 1.4.95

La release 1.4.95 non modifica le funzionalità didattiche. Introduce solo un micro-refactor interno del layer AJAX per separare i helper di request scope in un modulo AMD dedicato.

## Aggiornamento 1.4.97

La release 1.4.97 non modifica le funzionalità didattiche. Ripristina il modulo AMD `core/api/scope` estratto nella release precedente, includendo sia il sorgente sia il build AMD, e aggiunge il savepoint documentale mancante per la release 1.4.93.
...
La release 1.4.110 non modifica le funzionalita didattiche. Allinea il modulo AMD delle note personali al namespace `core/player/notes` usato dal facade `core/player`, includendo sorgente e build generati da Moodle.
