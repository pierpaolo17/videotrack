...
Reaction definitions are configured inside one collapsible Reactions section. The feature defaults to off and enabling it requires at least one complete active definition. The form renders at least four editable rows, keeps one spare row after existing active definitions and can be expanded with **Add reaction** up to the established cap of 30. Reaction controls and personal history appear only when both the feature and a definition are present. Users with `mod/videotrack:participate` can activate them while the player is playing or paused; before each interaction the client flushes current progress and uses the saved endpoint returned by the server. Reaction, note, bookmark and Forum timestamps are accepted only when covered by server-validated watched evidence; forward-seek permission never makes a `playstart` row authoritative. Report access is independent: dual-role participants remain interactive, while ordinary staff without the participation capability receive a disabled preview and cannot persist learner telemetry. Notes, bookmarks and Forum remain independently controlled by their own enablement and destination settings. External services enforce feature enablement, learner scope, ownership and maximum lengths. Deleted records retain a bounded tombstone until retention/erasure; private text is never copied into teacher aggregates.
...
Per-student reports may include identity, progress, reactions, acknowledgement date/snapshot and diagnostic counts only with individual-report access. Aggregate-only activity viewers cannot filter by learner and keep the configured minimum-user masking. Course and teacher dashboards aggregate accessible activities. The course dashboard uses separate renderers for text-only percentages and percentages with an accessible compact coverage bar. Instance Analytics uses exact bins and event summaries only when individual-report access covers the whole selected scope; otherwise it applies the configured threshold. Aggregate CSV/Excel/ODS downloads preserve that same privacy level. Their heading builder has explicit standard and event-summary entry points, so the selected export shape is visible at the call site. Individual CSV exports require the dedicated individual-export permission and explicit confirmation where personal data is present.
