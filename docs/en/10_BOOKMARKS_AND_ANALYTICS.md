...
The owner can list, replay, delete and CSV-export their bookmarks. Labels and exact timestamps are not shown to teachers. Per-student reports may show counts. Instance Analytics show exact aggregate event/distinct-user counts to authorised report viewers while keeping labels and exact bookmark timestamps private; the teacher-centric dashboard still uses `analyticsminusers` to mask small populations pending its dedicated review; the course dashboard is exact for authorised course-report viewers.
