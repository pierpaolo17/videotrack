...
VideoTrack started its browser-automation phase in release 1.7.45; release 1.7.108 adds deterministic Vimeo parity after the exact 1.7.107 gate passed. The maintainer Behat environment is operational on Moodle 5.0–5.3, and the suite uses unique native `<summary>` CSS selectors instead of ambiguous text clicks. The plugin ships a Moodle module generator under `tests/generator/lib.php` and browser scenarios under `tests/behat/`.
...
Release 1.7.112 added `completion_requirements_layout.feature`, and 1.7.113 aligned its selectors with both native and ARIA Moodle list renderings. The exact 1.7.113 suite passed 23 scenarios / 341 steps on Moodle 5.0–5.3. Release 1.7.114 added one geometry assertion: the grouped container must compute to a vertical flex column and each completion item must begin below the preceding one. Its exact gate passed 23 scenarios / 342 steps on Moodle 5.0 and 5.1, but exposed that Moodle 5.2/5.3 can place the completion-region marker on an ancestor of the list. Release 1.7.115 therefore targets VideoTrack's unique grouping marker directly and verifies separately that the marked list belongs to Moodle's completion region. The suite remains at 23 scenarios / 342 expected steps per branch and is not declared green before the exact-tree server gate.
