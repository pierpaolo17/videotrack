...
`classes/local/acknowledgement.php` owns statement activation, the SHA-256 identity of the current formatted statement and idempotent confirmation writes. `{videotrack_acknowledge}` stores only activity/user identifiers, the statement hash, the activity modification timestamp and confirmation time; it does not duplicate the teacher-authored statement. `view.php` uses a sesskey-protected POST form, enforces the optional final-second policy, stores an immutable viewing snapshot, refreshes aggregate completion state and asks Moodle to reevaluate custom completion. Changing statement text or format changes the hash and therefore requires a new confirmation. Backup/restore, Privacy API, retention, activity/course reset and teacher reports use the same current-hash rule. Since 1.6.21, Analytics builds per-activity current-hash clauses, applies the same capability/group scope as viewing data and exports a privacy-safe aggregate row without exposing individual confirmation dates.
