...
When enabled by a teacher, VideoTrack stores an explicit acknowledgement with the user id, activity and course-module ids, a non-reversible hash of the statement version, the activity modification timestamp and the confirmation timestamp. The full statement is not duplicated in the acknowledgement record. Confirmations are exported and erased through Moodle Privacy API, included in backups only with user data and deleted when the configured retention period expires.
