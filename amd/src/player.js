...
define([
    ...
    function writePlaybackRate(rate, label) {
        return Adapter.setPlaybackRate(rate, function(safeRate) {
            if (player && typeof player.setPlaybackRate === 'function') {
                return player.setPlaybackRate(safeRate);
            }
            return null;
        ...
        if (maxRate > 0 && currentRate > maxRate) {
            ...
            window.setTimeout(function() {
                if (player && typeof player.getPlaybackRate === 'function' && player.getPlaybackRate() > maxRate) {
                    writePlaybackRate(resetRate, (label || 'YouTube playback rate limit') + ' final');
                }
            }, 750);
            return resetRate;
        }
