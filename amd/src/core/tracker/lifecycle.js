/**
 * Provider-neutral tracker page lifecycle helpers.
 *
 * @module mod_videotrack/core/tracker/lifecycle
 */
define([
    'mod_videotrack/core/tracker/events',
    'mod_videotrack/core/tracker/state',
    'mod_videotrack/core/tracker/heartbeat'
], function(TrackerEvents, TrackerState, TrackerHeartbeat) {
    'use strict';
    ...
    /**
     * Execute an optional callback after tracker lifecycle cleanup.
     *
     * @param {Function|null} callback Optional callback.
     */
    function runAfterStop(callback) {
    ...
    /**
     * Stop tracking and optionally close an active segment.
     *
     * @param {Object} state Mutable player state.
     * @param {Object} handlers Lifecycle callbacks.
     * @param {string} reason Close reason.
     * @param {Object=} options Close options.
     */
    function closeThenStop(state, handlers, reason, options) {
        options = options || {};
        var hasOpenSegment = state && state.segmentstart !== null;
        if (!hasOpenSegment) {
            handlers.stop();
            runAfterStop(options.afterStop);
            return;
        }
        if (options.preferBeacon && handlers.sendBeacon) {
            if (handlers.sendBeacon()) {
                handlers.stop();
                runAfterStop(options.afterStop);
                return;
            }
        ...
        if (handlers.closeSegment) {
            Promise.resolve(handlers.closeSegment(reason)).catch(function(error) {
     ...
     * @param {Function=} options.hasPlayer Optional player availability callback.
     * @param {Function=} options.cancelPendingRequests Optional pending-request cleanup callback.
     * @returns {boolean} True when handlers were installed, false when already installed.
     */
    function installLifecycleHandlers(options) {
        options = options || {};
        var state = options.state;
        var cancelPendingRequests = typeof options.cancelPendingRequests === 'function' ? options.cancelPendingRequests : null;

        if (state && state.lifecycleHandlersInstalled) {
            uninstallLifecycleHandlers(state, cancelPendingRequests);
        }
        if (state) {
        ...
        var handlers = {
            closeSegment: typeof options.closeSegment === 'function' ? options.closeSegment : null,
            stop: typeof options.stopPolling === 'function' ? options.stopPolling : function() {
                stopPolling(state);
            },
            onHidden: typeof options.onHidden === 'function' ? options.onHidden : null,
            sendBeacon: typeof options.sendBeacon === 'function' ? options.sendBeacon : null,
            hasPlayer: typeof options.hasPlayer === 'function' ? options.hasPlayer : function() {
                return true;
            }
        };
        ...
        var onVisibilityChange = function() {
            ...
            closeThenStop(state, handlers, 'tab', {afterStop: handlers.onHidden});
        };

        var onPageHide = function() {
            emit(state, 'lifecycle:pagehide', {});
            closeThenStop(state, handlers, 'pagehide', {preferBeacon: true});
        };

        var onBeforeUnload = function() {
            if (!state || state.segmentstart === null ||
                    !safeBooleanCallback(handlers.hasPlayer, false, state, 'lifecycle:providererror')) {
                return;
            }
     ...
     *
     * @param {Object} state Mutable player state.
     * @param {Function=} cancelPendingRequests Optional pending-request cleanup callback.
     * @returns {boolean} True when handlers were removed.
     */
    function uninstallLifecycleHandlers(state, cancelPendingRequests) {
