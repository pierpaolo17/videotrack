...
class student_progress_reset extends \core\event\base {
    ...
    public function get_description(): string {
        ...
        $integrity = (int)($this->other['integrity'] ?? 0);
        $acknowledgements = (int)($this->other['acknowledgements'] ?? 0);
        return "The user with id '{$this->userid}' reset VideoTrack data for user with id " .
            ...
            "(deleted {$segments} segments, {$states} state records, {$events} interaction events, " .
            "{$integrity} integrity events and {$acknowledgements} acknowledgement records).";
    }
    ...
    protected function validate_data(): void {
        ...
        foreach (['segments', 'states', 'events', 'integrity', 'acknowledgements'] as $key) {
